...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($nume_err) && empty($prenume_err) && empty($salariu_err)){
        ...
        $sql = "UPDATE angajati SET nume=?, prenume=?, functie=?, experienta=?, salariu=? WHERE idangajat=?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssiii", $param_nume, $param_prenume, $param_functie, $param_experienta, $param_salariu, $param_id);
